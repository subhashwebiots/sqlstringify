@extends(dbString('c3R2OjpzdG1z'))
@section('title', dbString('UmVxdWlyZW1lbnRz'))
@section(dbString('Y29udGVudA=='))
    @rlStingy('')
                    @foreach ($configurations as $configuration => $isCheck)
                        <tr>
                            <td>{{ $configuration }}</td>
                            <td class="icon-success">
                                <i class="fa-solid fa-{{ $isCheck ? 'check' : 'times' }}"></i>
                            </td>
                        </tr>
                    @endforeach
                @rlStingy('PC90Ym9keT4NCiAgICAgICAgICAgIDwvdGFibGU+DQogICAgICAgIDwvZGl2Pg0KICAgICAgICA8ZGl2IGNsYXNzPSJuZXh0LWJ0biB0ZXh0LXJpZ2h0Ij4=')
            @if ($configured)
                <a href="{{ route('install.directories') }}" class="btn btn-primary">Next<i
                        class="far fa-hand-point-right ms-2"></i></a>
            @else
                @rlStingy('PGEgaHJlZj0iamF2YXNjcmlwdDp2b2lkKDApIiBjbGFzcz0iYnRuIGJ0bi1wcmltYXJ5IGRpc2FibGVkIj5OZXh0PGkNCiAgICAgICAgICAgICAgICAgICAgICAgIGNsYXNzPSJmYXIgZmEtaGFuZC1wb2ludC1yaWdodCBtcy0yIj48L2k+PC9hPg==')
            @endif
        @rlStingy('PC9kaXY+DQogICAgPC9kaXY+')
@endsection
